...
import { submitProfile, validateRegistration, lookupInstitution, submitBiometrics } from "./api";
...
export default function NYSCRegistration() {
  ...
  const handleNext = async () => {
    ...
    setSubmitting(true);
    try {
      if (currentStep === 0) { await submitProfile(profileData); setEmailSent(true); }
      if (currentStep === 1) await validateRegistration({ ...regData, email: profileData.email });
      if (currentStep === 2) { await lookupInstitution(lookupData); setLookupSuccess(true); }
      setCurrentStep(s => s + 1);
    } catch {
      setErrors((e: object) => ({ ...e, api: "Request failed. Please try again." }));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleFinish = async () => {
    ...
    try {
      await submitBiometrics({
        ...
        thumbprintLeft: biometrics.thumbprintLeft,
        thumbprintRight: biometrics.thumbprintRight,
        faceCapture: biometrics.faceCapture,
      });
      setComplete(true);
    } catch {
      setErrors((e: object) => ({ ...e, api: "Submission failed. Please try again." }));
    } finally {
      setSubmitting(false);
    }
  };
